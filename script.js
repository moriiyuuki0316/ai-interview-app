...
// --- OpenAI API 設定 ---
// !!! 注意 !!!
// 本番環境ではAPIキーをクライアントサイドに直接記述せず、必ずバックエンドサーバーを介してセキュアに管理してください。
// このデモは学習目的の簡易実装です。
const OPENAI_API_KEY = 'YOUR_OPENAI_API_KEY'; // ここにあなたのOpenAI APIキーを貼り付けてください
...
// --- OpenAI GPT API を使ってフィードバックを取得 ---
async function getGPTFeedback(question, answer) {
    if (!OPENAI_API_KEY || OPENAI_API_KEY === 'YOUR_OPENAI_API_KEY') {
        showError("OpenAI APIキーが設定されていません。script.jsを確認してください。");
    ...
    try {
        const response = await fetch(OPENAI_API_URL, {
            ...
            body: JSON.stringify({
                model: "gpt-4o", // または "gpt-3.5-turbo" など適切なモデルを選択
                messages: [
                    { role: "system", content: "あなたは経験豊富な就職面接官です。学生の面接回答に対して、改善点や良かった点、論理的思考、具体性、熱意などの観点から、丁寧かつ実践的なフィードバックを簡潔に提供してください。敬語を使い、ポジティブな言葉遣いを心がけてください。" },
                    { role: "user", content: `質問: ${question}\n\n回答: ${answer}\n\nこの回答に対するフィードバックをお願いします。` }
                ],
                max_tokens: 300,
        ...
        if (!response.ok) {
            ...
            throw new Error(`OpenAI API エラー: ${response.status} ${response.statusText} - ${errorData.error ? errorData.error.message : '不明なエラー'}`);
        }
        ...
        showError(`AIフィードバックの取得中にエラーが発生しました: ${error.message}`);
        return "AIフィードバックの取得に失敗しました。APIキーまたはネットワークを確認してください。";
    }
}
